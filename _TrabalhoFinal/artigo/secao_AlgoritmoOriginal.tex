...
No grafo de exemplo com seis vértices, a Solução Original encontrou uma coloração válida após 141 tentativas, com tempo registrado de $0{,}001601$ s e pico de memória de $6{,}12$ KiB. Em grafos completos $K_n$, usados no Grupo 4 para criar instâncias difíceis quando $n>3$, o algoritmo esgotou $3^n$ combinações nas instâncias não 3-coloríveis. Essa característica é relevante: instâncias sem solução podem ser mais custosas do que instâncias satisfazíveis quando uma solução aparece cedo na ordem de enumeração. O método, portanto, é apropriado como linha de base experimental e como implementação de referência, mas não escala para grafos maiores porque não aprende com rejeições anteriores nem antecipa conflitos locais.
